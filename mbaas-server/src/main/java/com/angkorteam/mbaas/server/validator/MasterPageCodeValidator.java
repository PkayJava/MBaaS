 ...
 */
public class MasterPageCodeValidator extends JooqValidator<String> {

    private static final List<Character> CODES = Arrays.asList('a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u', 'v', 'w', 'x', 'y', 'z');

    private final String applicationCode;

    public MasterPageCodeValidator(String applicationCode) {
    ...
    public void validate(IValidatable<String> validatable) {
        ...
        if (code != null && !"".equals(code)) {
            ...
            int count = jdbcTemplate.queryForObject("SELECT COUNT(*) FROM " + Jdbc.MASTER_PAGE + " WHERE " + Jdbc.MasterPage.CODE + " = ?", int.class, code);
            if (count > 0) {
