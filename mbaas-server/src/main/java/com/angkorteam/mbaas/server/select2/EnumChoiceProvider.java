...
public class EnumChoiceProvider extends SingleChoiceProvider<Map<String, Object>> {
    ...
    public List<Option> query(String term, int page) {
        ...
        List<Map<String, Object>> enumRecords = jdbcTemplate.queryForList("SELECT * FROM " + Jdbc.ENUM + " WHERE LOWER(" + Jdbc.Enum.NAME + ") LIKE LOWER(?) LIMIT " + ((page - 1) * LIMIT) + "," + LIMIT, term + "%");
        for (Map<String, Object> enumRecord : enumRecords) {
            options.add(new Option((String) enumRecord.get(Jdbc.Enum.ENUM_ID), (String) enumRecord.get(Jdbc.Enum.NAME)));
        }
        return options;
    }
